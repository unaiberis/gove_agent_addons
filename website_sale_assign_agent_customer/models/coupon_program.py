...
class CouponProgram(models.Model):
    ...
    def _keep_only_most_interesting_auto_applied_global_discount_program(
        ...
        if not order and (groups or no_group_programs):
            ...
            order = (
                website.sale_get_order(
                    force_pricelist=_pricelist.id, update_pricelist=True
                )
                if website and _pricelist
                else None
            )
            if not order:
        ...
        if order:
            if request.env["agent.partner"].search([('agent_id','=',request.env.user.sudo().partner_id.id)], limit=1).customer_id_chosen_by_agent:
                order.partner_id = (
                    request.env["agent.partner"].search([('agent_id','=',request.env.user.sudo().partner_id.id)], limit=1).customer_id_chosen_by_agent
                )
