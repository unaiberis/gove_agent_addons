    ...
    def _send(self, auto_commit=False, raise_exception=False, smtp_session=None):
        ...
        for mail_id in self.ids:
            ...
            try:
                ...
                if (
                    self.env.company.resend_mail
                    and mail.model == "sale.order"
                    and "Surflogic" in mail.subject
                ):
                    # Code changed to send emails to surflogic.com
                    # Adding 'info@surflogic.com' as an additional recipient for the same email
                    _logger.info(
                        f"logger_gove - email_list: {pprint.pformat(email_list)}"
                    )
                    for email_entry in email_list[:]:
                        ...
                        if "info@surflogic.com" not in email_to:
                            ...
                            _logger.info(
                                f"logger_gove - info@surflogic.com added email_list: {pprint.pformat(email_list)}"
                            )
                    ...
                    for partner in mail.recipient_ids:
                        ...
                        for agent in agents:
                            if partner.id in agent.agent_customers.ids:
                                ...
                                _logger.info(
                                    f"logger_gove - agent {agent} added - email_list: {pprint.pformat(email_list)}"
                                )
