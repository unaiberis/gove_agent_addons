...
class ResPartner(models.Model):
    ...
    customer_from_woo = fields.Boolean("Customer Imported from WooCommerce")

    @api.model
    def woo_create_contact_customer(self, vals, instance=False):
        partner = super().woo_create_contact_customer(vals, instance)
    ...
    def woo_create_or_update_customer(
        self, customer_val, instance, parent_id, partner_type, customer_id=False
    ):
        partner = super().woo_create_or_update_customer(
            customer_val, instance, parent_id, partner_type, customer_id
        )
        partner.write({"customer_from_woo": True})
        _logger.info(
